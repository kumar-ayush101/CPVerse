    ...
    <div className="min-h-screen  text-white p-6">
      ...
      <ul className="space-y-6 max-w-4xl w-full">
        {doubts.map((doubt) => (
          <li
            key={doubt._id}
            className="bg-black border border-blue-500 rounded-xl p-5 shadow-lg"
          >
            <p className="text-white text-lg mb-4">{doubt.text}</p>

            
            {replies[doubt._id]?.length > 0 && (
              <div className="space-y-2 mb-3">
                {replies[doubt._id].map((reply, idx) => {
                  ...
                  return (
                    ...
                    >
                      {isExpanded ? reply.text : `${reply.text.slice(0, 30)}...`}
                    </div>
                  );
                })}
              </div>
